...
<style type="text/css">l
	...
	#code{ width:33%;height:50px;line-height:50px;font-size:16px;}
...
<SCRIPT type="text/javascript" src="__JS__jquery-1.8.1.min.js"></SCRIPT>
...
$(document).ready(function(){
	$(".schoolno").blur(function(){
		...
		$.ajax({
			type:"POST",
			url:"Register/check_iscode",
			data:"schoolno="+schoolno,
			dataType:"json",
			success:function(data){
				if($.trim(data['iscode'])=='1'){
					var content='onclick="this.src=this.src+'+"'?'"+'+Math.random();return false;"';
					$(".code").append('<span>请输入验证码：</span><input name="code" id="code" type="text"/><img src="Getcode/getcode?schoolno='+schoolno+'"'+content+' />');	
					//alert('<span>请输入验证码：</span><input name="code" id="code" type="text"/><img src="Getcode/getcode?schoolno='+schoolno+'"'+content+' />');
					}else{	 
...
<div class="sty">
	...
	<span>学校：</span>
	<select name='schoolno' class="schoolno" >
